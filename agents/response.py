...
class ChunkData(BaseModel):
    ...
    def get_last_ai_message(self) -> str:
        ...
        ai_messages_with_content = [
            msg
            for msg in first_agent.messages
        ...
        for agent_id, agent_data in raw_data.items():
            ...
            for msg_data in agent_data["messages"]:
                ...
                if hasattr(msg_data, "__class__") and hasattr(msg_data, "content"):
                    ...
                    msg_dict = (
                        msg_data if isinstance(msg_data, dict) else msg_data.__dict__
                    )
                    ...
                    if isinstance(msg_dict.get("content"), list):
                        ...
                        or "usage_metadata" in msg_dict
                        or msg_dict.get("response_metadata", {}).get("finish_reason")
                        or msg_dict.get("response_metadata", {}).get("model_name")
                    ):
...
if __name__ == "__main__":
    ...
    try:
        ...
        ai_messages = [
            msg for msg in chunk.data["agent1"].messages if isinstance(msg, AIMessage)
        ]
        if ai_messages:
            ...
            for i, ai_msg in enumerate(ai_messages):
                ...
                if ai_msg.tool_calls:
                    print(
                        f"Tool calls: {len(ai_msg.tool_calls)} - {[tc.name for tc in ai_msg.tool_calls]}"
                    )
                if ai_msg.usage_metadata:
                    print(
                        f"Token usage: {ai_msg.usage_metadata.total_tokens} total tokens"
                    )
        else:
